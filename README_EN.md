...
- **Concurrent Multi-conversation Streaming** — Stream tasks are isolated per **message** (`Map<messageId, StreamTask>`): different conversations — and even different branches of the same conversation — can stream / pause / resume independently without interfering; switching away keeps the original stream running in the background with no cross-writes
- **AI First-answer Auto-pin** — On the AI's first output (thinking or main text, triggered as the stream starts) `updatedAt` is refreshed and persisted immediately, auto-pinning the conversation to the top of the sidebar; each stream pins only once, avoiding stale ordering during thinking
...
- **Delete Single Message** (`deleteMessage`): breadth-first traversal along parentId collects the message + all descendants → precisely cleans up the corresponding stream tasks by the subtree id set (including paused / in-progress tasks in other branches, `abort()` aborts the underlying request) → removed from DB and memory cache in sync → rebuilds the list; auto-resets to a new conversation when everything is deleted
...
- **Smart Scroll (staged follow)**: streaming follow switches strategy by stage — thinking stage (multi-stage thinking / search interleaving, fast increments that animation can't keep up with) pins to bottom immediately (`scrollToBottom`); main-text generation stage (relatively slow) uses a 50ms debounce + 200ms EaseOut animation (`scrollToBottomSmooth`, animated via a `getUIContext().animateTo` executor injected by the page) to keep the smooth feel; only one animation group at a time to avoid animation pile-up jitter
...
- **Instant re-pin on item height change**: ListItem `onAreaChange` reports the height delta (`onListItemHeightChanged`); when async Markdown rendering grows an item, the list re-pins to bottom immediately — faster than fixed-interval retries and idle-free; during the main-text generation stage it's left to the smooth animation (skipped so as not to interrupt it), while during the thinking stage (`isTailGeneratingContent()` returns false) it pins immediately to keep up with fast increments
...
- **Stable thinking-line keys**: the ForEach key for per-line reasoning rendering uses only "message id + line index" (reasoning is append-only, so the index stays constant for existing lines and increments for new ones — naturally unique and stable); previously the key included `line.length` / `substring`, so during streaming thinking the last line's key changed on every increment, causing that line to be repeatedly destroyed/rebuilt and manifesting as page jitter
