...
- **LazyForEach** 懒加载 + 50ms 流式节流；`cachedCount` 预加载 + 保持可见内容位置稳定，缓解快速滑动 / 流式增高的视口跳动
- **智能滚动**：思考阶段即时钉底、正文阶段 50ms 防抖 + 200ms 平滑动画；`scrollEdge` 优先定位；加载后多次钉底重试；变体切换用「锚点记录 + 增量替换 + 高度差补偿」防闪跳；用户拖动期间暂停流式 UI 写入，到达底部时区分用户手势与代码滚动
